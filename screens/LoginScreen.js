...
// function Login(
// { route, navigation, token, dispatchSetIsLogin, dispatchSetEmail,
// dispatchSetName, dispatchSetPosition }) {
//   // router로 전달 받은 param
...
//     const setStore = async (res) => {
//       console.log(token, res);
//       if(token) {  // 토큰이 있으면
//         await dispatchSetIsLogin(true);
//         await dispatchSetEmail(res.email);
//         await dispatchSetName(res.username);
//         await dispatchSetPosition(res.position);
//       }
//     }
...
//                   // store position에 서버로 부터 받은 정보가 저장되어야 한다.
...
//                   console.log(token);
//                   navigation.navigate('Profile'); // 나중에 Mentee home 화면으로 바꿔줄 예정
//                   // state값들을 초기화 해줘야 한다.
